...
def generate_trace_donor():
    ...
    if not Definitions.NO_SYM_TRACE_GEN:
        ...
        exploit_command = " ".join(Definitions.VALUE_EXPLOIT_A.split(" ")[1:])
...
def generate_trace_target():
    ...
    if not Definitions.NO_SYM_TRACE_GEN:
        ...
        binary_dir, binary_name = Converter.convert_binary_to_llvm(binary_path)
        exploit_command = " ".join(Definitions.VALUE_EXPLOIT_C.split(" ")[1:])
        KleeExecutor.generate_trace(exploit_command,
                                    binary_dir,
                                    binary_name,
                                    Definitions.VALUE_PATH_POC,
